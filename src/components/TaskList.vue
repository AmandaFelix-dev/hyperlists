  ...
  <section class="task-list" v-if="tasks.length">
    <div v-for="task in tasks" :key="task.id" class="task-card">
      <div class="left">
        ...
        <div class="desc">
          <p v-if="!isEditing(task.id)">{{ task.description || '— sem descrição —' }}</p>
          <textarea v-else v-model="edited[task.id].description"></textarea>
        </div>
        <div class="spotify" v-if="task.spotify && !isEditing(task.id)">
          <a :href="task.spotify" target="_blank" rel="noreferrer">Abrir link do Spotify</a>
          <div v-if="embedSrc(task.spotify)" class="embed">
            <iframe :src="embedSrc(task.spotify)" width="100%" height="80" frameborder="0" allow="autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture"></iframe>
          </div>
        </div>
        <div class="spotify-edit" v-else-if="isEditing(task.id)">
          <input v-model="edited[task.id].spotify" placeholder="Link do Spotify (opcional)"/>
        </div>
      </div>
      ...
      <div class="right">
        ...
        <div v-else>
          <button @click="saveEdit(task.id)">Salvar</button>
          <button class="secondary" @click="cancelEdit(task.id)">Cancelar</button>
        </div>
...
function isEditing(id){ return !!editing[id] }
...
function emitDelete(id){
  if(confirm('Deletar tarefa?')) emit('delete', id)
}
...
.task-list{ display:flex; flex-direction:column; gap:10px;
  .task-card{ display:flex; gap:12px; padding:12px; border:1px solid #eee; border-radius:8px; align-items:flex-start;
    .left{ flex:1;
      ...
      .spotify{ margin-top:8px;
        a{ display:inline-block; margin-bottom:8px; }
        .embed{ margin-top:6px; border-radius:6px; overflow:hidden; }
      }
    }
    .right{ display:flex; flex-direction:column; gap:6px;
      ...
      .danger{ background:#ffdddd; border:1px solid #ffaaaa; }
    }
  }
}
.empty{ color:#777; text-align:center; margin-top:12px; }
</style>
